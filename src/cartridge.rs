/* 
    ROM consists of two banks of memory

    PRG ROM - Contains the code
    CHR ROM - Contains the visual graphics

    PRG ROM - connects to the CPU
    CHR ROM - connects to the PPU

    CPU can't access CHR ROM directly
    PPU can't access PRG ROM

    We won't be working with cartridges but ROM dumps

    We wil use iNES format

    iNES format:
    - 16 byte header
    - 512 byte Trainer - Optional
    - Section containing PRG ROM Code
    - Section containing CHR ROM code

    HEADER:
    - String NES^Z is used to reconize .NES files
    - Number of 16kB ROM banks (PRG ROM)
    - Number of 8K VROM Banks (CHR ROM)
    - Control Byte 1
    - Control Byte 2
    - Size of PRG RAM in 8kb units
    - Reserverd Bytes, must be 0
    ...
    - CHR ROM
    - Mapper Type
    - Mirroing Type: Horizontal, Vertical, 4 Screen

    We will use mapper 0, meaning no mapper
*/ 
...
impl Rom {
    pub fn new(raw: &Vec<u8>) -> Result<Rom, String> {
        ...
        if ines_ver != 0 {
            return Err("NES@.0 format is not supported".to_string());
        }
