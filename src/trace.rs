...
pub fn trace(cpu: &CPU) -> String {
    let ref opscode: HashMap<u8, &'static opcodes::OpCode> = *opcodes::OPCODES_MAP;

    let code = cpu.mem_read(cpu.program_counter);
                    
    

    let ops = opscode.get(&code).unwrap();
    ...
    let tmp = match ops.len {
        ...
        2 => {
            ...
            match ops.mode {
                ...
                _ => panic!( "Unexpected addressing mode {:?} has ops-len2. code{:02x}", ops.mode, ops.code),
            }
        },
    ...
    let asm_str = format!("{:04x}  {:8} {: >4} {}", begin, hex_str, ops.mnemonic, tmp).trim().to_string();
